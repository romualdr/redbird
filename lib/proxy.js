    ...
    _this._getTarget(src, req).bind(_this).then(function (target) {
      if (target){
        if (shouldRedirectToHttps(_this.certs, src, target, _this)) {
          ...
          proxy.web(req, res, { headers: target.headers, target: target, secure: (proxy.options && proxy.options.secure) || true});
        }
      } else {
...
ReverseProxy.buildRoute = function (route) {
  ...
  if (entry) {
    let headers = Object.assign({}, entry.headers || {}, route.headers || {});
    if (!Object.keys(headers))
      headers = undefined;
    return Object.assign({}, entry, { headers });
  }
  ...
  if (route.headers)
    routeObject = Object.assign(routeObject, { headers: route.headers });
  return routeObject
};
